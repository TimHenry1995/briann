...
from experiments.vggish import data_loader as evdl
...
for i, dimensionality in enumerate([[1, 96, 64], [64, 96, 64], [64, 48, 32], [128, 48, 32], [128, 24, 16], [256, 24, 16], [256, 24, 16], [256, 12, 8], [512, 12, 8], [512, 12, 8], [512, 6, 4], [4096], [4096], [128], [128]]):
    _time_frame_accumulators[i] = bnc.TimeFrameAccumulator(initial_time_frame=bnc.TimeFrame(state=torch.zeros([_batch_size] + dimensionality), time_point=-float('inf')), sustain=0.0001) 

# Set connections
connection_list = [None] * 14
for i in range(len(connection_list)):
    connection_list[i] = bnc.Connection(index=i, from_area_index=i, to_area_index=i+1, input_time_frame_accumulator=_time_frame_accumulators[i], transformation=torch.nn.Identity())

connection_list.append(bnc.Connection(index=14, from_area_index=13, to_area_index=7, input_time_frame_accumulator=_time_frame_accumulators[13], transformation=torch.nn.Sequential(
...
# Construct briann
_briann = bnc.BrIANN(name="VGGish", areas=_areas, connections=_connections)

sinusoids = evdl.Sinusoids(instance_count=100, duration=10.0, sampling_rate=16000, frequency_count=128, noise_range=0.5)
_data_iterator = iter(torch.utils.data.DataLoader(dataset=evdl.VGGishSpectrograms(waveforms=sinusoids), 
