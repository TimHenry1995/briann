...
public:
    void add(int num) {
        while (true) {
            std::unique_lock<std::mutex> locker(mu); // Lock the mutex for this thread
...
private:
    Buffer* buffer_; // The * indicates that this is a pointer to a Buffer object
    std::string name_; // Here, we use no *, as we are not passing a pointer to a string object
};
